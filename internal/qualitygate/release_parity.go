...
func releaseParity(ctx context.Context, root string) error {
	parent, err := os.MkdirTemp("", "starter-smtp-release-parity-*")
	if err != nil {
...
func validateReleaseParity(
	...
	base := "starter-smtp_" + strings.TrimPrefix(rehearsalVersion, "v")
	...
	if err := validateReleaseArchiveParity(
		...
		"starter-smtp_"+strings.TrimPrefix(rehearsalVersion, "v")+"/",
		filepath.Join(retainedRoot, archiveName),
		"starter-smtp-"+strings.TrimPrefix(rehearsalVersion, "v")+"/",
	); err != nil {
		return err
	}
...
func validateReleaseSBOMParity(centralContent, retainedContent []byte) error {
	...
	baseNamespace := "https://github.com/spice-framework/starter-smtp/releases/" +
	...
	if !found {
		return errors.New("central release SBOM has no exact SMTP root package")
	}
...
func validateCentralSBOMProvenance(document releaseSBOM, baseNamespace string) error {
	if document.Name != "starter-smtp "+rehearsalVersion ||
...
func validateRetainedSBOMProvenance(document releaseSBOM, baseNamespace string) error {
	if document.Name != "Spice SMTP Starter "+rehearsalVersion ||
		...
		return errors.New("retained release SBOM provenance does not match the SMTP builder")
	}
	return nil
}
